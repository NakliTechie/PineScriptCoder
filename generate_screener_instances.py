...
def get_global_commodity_tickers():
    ...
    Returns:
        list: List of global commodity, currency, and crypto tickers
    """
    return [
        'XAUUSD',    # Gold
        'XAGUSD',    # Silver
        ...
        'CLUSD',     # Crude Oil
        'NGUSD',     # Natural Gas
        'GCUSD',     # Gold (alternative)
        'SIUSD',     # Silver (alternative)
        'EURUSD',    # Euro/USD
...
def generate_screener_instances(market, num_instances, base_script_name, script_title):
    """
    ...
    Args:
        ...
        base_script_name (str): Base name for the script (e.g., 'KURU')
        script_title (str): Title for the script (e.g., 'Kurutoga Screener')
    """
...
indicator('{base_script_name} - {instance_num}', overlay = true, max_lines_count = 500, max_labels_count = 500, max_boxes_count = 500)
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
{chr(10).join(enable_flags)}

//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
// SYMBOLS - Tickers
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
{chr(10).join(ticker_inputs)}

// --- Script Name ---
string SCRIPT_NAME = "{base_script_name}" 
...
if barstate.isfirst
{chr(10).join(array_sets)}

// Update signal_arr on every bar
{chr(10).join(signal_array_sets)}
...
if barstate.islast
    ...
    for i = 0 to 39 by 1
        if array.get(u_arr, i) 
            ...
            if str.contains(current_signal, "B")
                countBuy += 1
                signal_bg_col := colorBullishBG
            else if str.contains(current_signal, "S")
                countSell += 1
...
def main():
    if len(sys.argv) != 5:
        print("Usage: python3 generate_screener_instances.py <market> <num_instances> <base_script_name> <script_title>")
        print('Example: python3 generate_screener_instances.py us 10 KURU "Kurutoga Screener"')
        sys.exit(1)
    ...
    success = generate_screener_instances(market, num_instances, base_script_name, script_title)
    if not success:
        sys.exit(1)
